...
function showNextWord() {
  ...
  if (wordIndex < words.length) {
    ...
    wordIndex++;
    setTimeout(showNextWord, 500); // delay between words (500ms)
  } else {
    // Whole line shown; wait and then go to next line
    setTimeout(() => {
      container.textContent = ''; // clear
      wordIndex = 0;
      lineIndex = (lineIndex + 1) % lines.length; // cycle lines
      showNextWord();
    }, 2000); // wait 2 seconds before next line animation starts
  }
}
